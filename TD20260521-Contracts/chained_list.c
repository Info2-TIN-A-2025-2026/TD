...
int list_remove_elem(chained_list *l, size_t pos)
{
    ...
    if (n->prev)
        ...
        l->first = n->next; /* removed head */

    if (n->next)
        n->next->prev = n->prev;
    else
        l->last = n->prev; /* removed tail */
...
int list_save(chained_list l, const char *filename)
{
    ...
    fwrite(&(l.count), sizeof(size_t), 1, f);
    node *cur = l.first;
    while (cur)
    {
...
int list_load(chained_list *l, const char *filename)
{
    ...
    *l = list_init();
    ...
    {
        element e;
        int matched = fread(&e, sizeof(element), 1, f);
        if (matched == 1)
        {
